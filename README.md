...
全局索引提取器节点，无论输入是单个Batch还是多个List，将其视为一个连续的图片序列，根据索引提取其中唯一的一张图片。实现"多输入 -> 指定索引 -> 单输出"的功能。

### 功能特点

- **全局索引**：无论输入是Batch还是List，都视为连续序列统一处理
- **灵活输入**：支持单个Batch或多个List组合输入
- **精确提取**：根据索引值精确提取指定位置的图片
- **智能边界处理**：自动处理越界情况，负数索引修正为0，超出范围自动取最后一张

### 输入参数

- **images** (IMAGE): 输入的图片，可以是单个Batch或多个List
- **image_index** (INT): 要提取的图片索引（从0开始），范围0-10000，默认0

### 输出

- **image** (IMAGE): 提取的单张图片，形状为 [1, H, W, C]

### 使用场景

#### 场景 A：Batch 输入
- **输入**：Batch Size = 4 的一组图
- **输入 Index**：2
- **输出**：第 3 张图（索引从0开始，索引2对应第3张）
- **Preview 结果**：1张图

#### 场景 B：List 列表输入
- **输入**：List 列表（第一组 2 张猫图，第二组 3 张狗图，共 5 张）
- **输入 Index**：3
- **逻辑**：跳过前 2 张猫图（索引0-1），在第二组中取第 2 张狗图（索引2，相对位置为索引3-2=1，即第二组的第2张）
- **输出**：1 张狗图
- **Preview 结果**：1张图

### 工作原理

节点将所有输入的图片（无论是Batch还是List）视为一个连续的序列，按照输入顺序进行全局编号。当指定索引时，节点会：
1. 计算所有输入图片的总数
2. 遍历所有输入组，累加每组的图片数量
3. 找到目标索引所在的组
4. 在该组内计算相对位置并提取对应图片
5. 返回形状为 [1, H, W, C] 的单张图片
